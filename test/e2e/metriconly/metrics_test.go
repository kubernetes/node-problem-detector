...
var _ = ginkgo.Describe("NPD should export Prometheus metrics.", func() {
	...
	ginkgo.Context("When ext4 filesystem error happens", func() {
		...
		ginkgo.It("NPD should remain healthy", func() {
			npdStates, _ := instance.RunCommandOrFail("sudo systemctl show node-problem-detector -p ActiveState -p SubState")
			Expect(npdStates).To(ContainSubstring("ActiveState=active"), "NPD is no longer active: %v", npdStates)
			Expect(npdStates).To(ContainSubstring("SubState=running"), "NPD is no longer running: %v", npdStates)
		})
	})
