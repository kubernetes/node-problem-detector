...
func NewProblemMetricsManagerStub() (*ProblemMetricsManager, *metrics.FakeInt64Metric, *metrics.FakeInt64Metric) {
	fakeProblemCounter := metrics.NewFakeInt64Metric("npd_problem_counter", metrics.Sum, []string{"reason"})
	fakeProblemGauge := metrics.NewFakeInt64Metric("npd_problem_gauge", metrics.LastValue, []string{"type", "reason"})
