...
func (p *Plugin) run(rule types.CustomRule) (exitStatus cpmtypes.Status, output string) {
	...
	if cmd.ProcessState.Sys().(syscall.WaitStatus).Signaled() {
		output = fmt.Sprintf("%s. %s", cmd.ProcessState.String(), output)
	}
